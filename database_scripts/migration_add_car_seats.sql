-- 1. Add car_seats to driver_applications and profiles
ALTER TABLE public.driver_applications ADD COLUMN IF NOT EXISTS car_seats INTEGER;
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS car_seats INTEGER;

-- 2. Update the trigger that runs BEFORE INSERT ON profiles
...
-- 3. Update the trigger that runs AFTER UPDATE ON driver_applications
